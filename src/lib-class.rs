...
impl BleHeartRate {
  ...
  #[napi]
  pub async fn start(&self, callback: ThreadsafeFunction<(u32, u32), (), (u32, u32), false>) {
    let central_state = self.central.adapter_state().await.unwrap();
    println!("CentralState: {:?}", central_state);

    let events = match self.central.events().await {
      Ok(e) => e,
      Err(_) => {
        return;
      }
  ...
  pub async fn stop(&self) {
    self.central.stop_scan().await.unwrap();
  }
}